...
import com.prompthub.notification.infra.messaging.kafka.event.OrderPaidPayload;
...
public class OrderPaidEventHandler {
    ...
    private static final String TITLE = "결제가 완료되었습니다.";
    ...
    public void handle(EventMessage<JsonNode> message) {
        OrderPaidPayload payload = payload(message);
        ...
        StoredNotification stored = notificationCommandService.createIfAbsent(new CreateNotificationCommand(
            message.eventId(), payload.buyerId(), NotificationType.ORDER_PAID, TITLE,
            "주문 결제가 완료되었습니다.", "ORDER", payload.orderId(), CONSUMER_GROUP, occurredAt
        ));
        if (stored.created()) {
    ...
    private OrderPaidPayload payload(EventMessage<JsonNode> message) {
        try {
            OrderPaidPayload payload = objectMapper.treeToValue(message.payload(), OrderPaidPayload.class);
            if (payload.orderId() == null || payload.buyerId() == null || payload.paidAt() == null || payload.totalOrderAmount() < 0) {
                throw new IllegalArgumentException("ORDER_PAID 필수 필드가 누락되었습니다.");
            }
            return payload;
        } catch (JacksonException exception) {
            throw new IllegalArgumentException("ORDER_PAID payload를 변환할 수 없습니다.", exception);
        }
    }
