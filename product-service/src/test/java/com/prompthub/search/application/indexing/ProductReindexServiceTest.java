...
class ProductReindexServiceTest {
	...
	void reconcileAll_ON_SALE_family는_upsert_대상에_담는다() {
		...
		given(familyStatsResolver.buildFamilyUpsertInput(List.of(onSale), onSale, 4.0, null)).willReturn(expectedInput);
	...
	void reconcileAll_family_수와_무관하게_조회는_상수_건이다() {
		...
		given(familyStatsResolver.buildFamilyUpsertInput(anyList(), any(), anyDouble(), any()))
	...
	void reconcileChanged_변경된_family만_upsert한다() {
		...
		given(familyStatsResolver.buildFamilyUpsertInput(List.of(onSale), onSale, 5.0, null)).willReturn(expectedInput);
