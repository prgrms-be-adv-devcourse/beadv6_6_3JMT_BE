...
public class ProductSearchEventProcessor {
	...
	@Transactional
	public void processProductChanged(UUID eventId, LocalDateTime occurredAt, UUID familyRootId) {
	...
	@Transactional
	public void processRemovalCandidate(UUID eventId, LocalDateTime occurredAt, String eventType, UUID productId) {
		if (alreadyProcessed(eventId)) {
			return;
		}
	...
	private void reconcileFamily(UUID familyRootId) {
		...
		if (currentOnSale.isEmpty()) {
			productSearchIndexPort.bulkReconcile(List.of(), List.of(familyRootId));
			return;
		}
		...
		FamilyUpsertInput input = familyStatsResolver.buildFamilyUpsertInput(
			members, representative, averageRating, embedding);
		productSearchIndexPort.upsert(input);
	}
