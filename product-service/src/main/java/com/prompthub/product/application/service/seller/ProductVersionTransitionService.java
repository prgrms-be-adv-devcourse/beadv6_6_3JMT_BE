 ...
 * 책임 분리). PATCH는 기존 row를 SUPERSEDED로 교대시키고 PRODUCT_CHANGED를 발행하며, MAJOR는
 * 기존 row를 그대로 두고 검수 요청을 발행한다. "새 버전을 어떻게 반영하는가"라는 한 가지
 * 이유로만 바뀐다 — 어떤 버전 유형인지 판정하는 일은 {@link ProductVersionChangePolicy}가 맡는다.
 */
@Service
...
public class ProductVersionTransitionService {
	...
	public Product transitionToNextVersion(
		...
		ProductContent stored, String changeReason, UUID familyRootId
	) {
		...
		if (versionType == ProductVersionType.PATCH) {
			...
			productEventPublisher.publishProductChanged(familyRootId);
		} else {
			productRepository.save(next);
