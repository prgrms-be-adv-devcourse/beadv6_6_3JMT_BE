...
이번 재설계는 product-service 자신의 구조를 개선하는 데 목적이 있다. 그중 액션 단위 skill 구성과 `evals/`, `agents/rule-checker`
...
CLAUDE.md 체크리스트 확인
  ...
  → create-github-pr
       ...
       └─ agents/rule-checker: verify-rules + sync-product-docs + 모듈 경계 + 이슈/브랜치 확인 4가지 종합 게이트
