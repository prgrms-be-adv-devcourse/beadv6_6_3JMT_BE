...
- 이 컨벤션은 **product-service가 새로 추가하는 gRPC 메서드**에 적용한다. product-service가
  ...
  현재 4개 메서드: `GetSellerStats`(셀러 통계, 옛 `CountBySeller`), `GetOrderSnapshots`,
  `GetCartSnapshots`, `GetProductContent` — 모두 `Get~` 규칙에 부합. `GetProductsByIds`는
  실제 호출자가 없어 제거했다(#431) — 대체 용도(찜 목록 상품 카드 조회)는 공개 REST
  `GET /products/by-ids`로 노출한다.
