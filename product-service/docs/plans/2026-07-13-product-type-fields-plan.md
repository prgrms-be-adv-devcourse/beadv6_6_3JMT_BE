...
**Goal:** product 상품 유형(PROMPT/NOTION/PPT/EXCEL)별로 서로 다른 산출물 필드(`file_url`, `content_file_url`)를 추가하고, 유형에 맞지 않는 필드 조합을 도메인에서 거부한다.

**Architecture:** `product` 테이블에 nullable 컬럼 2개를 두는 sparse-column 방식. 유형별 정합성은 `Product` 도메인 팩토리에서 검증한다. 서비스는 유형 분기 없이 `file_url`을 스토리지 키로(기존 thumbnail 흐름 재사용), `content_file_url`을 외부 링크 원문으로 처리하고, 도메인 검증이 상호배타성을 보장한다.
...
- 유형별 필드 매트릭스: PROMPT→`content` / PPT·EXCEL→`file_url` / NOTION→`content_file_url` (각 1개, 나머지 null)
- 불일치/누락 = `ProductException(PRODUCT_TYPE_FIELD_MISMATCH)` → 400
- 컬럼 순서: 팩토리 시그니처는 `content` 바로 다음에 `fileUrl`, `contentFileUrl` 삽입
...
`file_url`/`content_file_url`이 요청→엔티티→저장까지 흐르게 만든다. 유형 검증은 Task 2. 이 태스크 종료 시 모듈이 컴파일되고 기존 테스트가 통과해야 한다.
...
**Interfaces:**
- Produces:
  - `ProductCreateRequest(String title, String productType, String model, String desc, Integer amount, String content, String fileUrl, String contentFileUrl, String thumbnailUrl, List<String> imageUrls, List<String> tags)` — `content`에서 `@NotBlank` 제거
  - `ProductUpdateRequest(String title, String productType, String model, String desc, Integer amount, String content, String fileUrl, String contentFileUrl, String thumbnailUrl, List<String> imageUrls, List<String> tags, String changeReason, String versionType)`
  - `Product.create(UUID id, UUID sellerId, ProductType productType, String name, String description, String model, AmountType amountType, int amount, String thumbnailUrl, List<String> imageUrls, String content, String fileUrl, String contentFileUrl, List<String> tags)`
  - `Product.update(ProductType productType, String name, String description, String model, AmountType amountType, int amount, String thumbnailUrl, List<String> imageUrls, String content, String fileUrl, String contentFileUrl, List<String> tags, String changeReason, boolean isMajor)`
  - `Product.nextVersion(boolean isMajor, ProductType productType, String name, String description, String model, AmountType amountType, int amount, String thumbnailUrl, List<String> imageUrls, String content, String fileUrl, String contentFileUrl, List<String> tags, String changeReason)`
  - `Product.getFileUrl()`, `Product.getContentFileUrl()` (Lombok `@Getter`)
...
`create(...)` 시그니처를 `String content, List<String> tags` → `String content, String fileUrl, String contentFileUrl, List<String> tags`로 바꾸고, 본문에서 `product.content = content;` 아래에 대입 추가:
...
`update(...)` 시그니처를 `String content, List<String> tags, String changeReason, boolean isMajor` → `String content, String fileUrl, String contentFileUrl, List<String> tags, String changeReason, boolean isMajor`로 바꾸고 `this.content = content;` 아래에 추가:
...
`nextVersion(...)` 시그니처를 `String content, List<String> tags, String changeReason` → `String content, String fileUrl, String contentFileUrl, List<String> tags, String changeReason`로 바꾸고 `next.content = content;` 아래에 추가:
...
- [ ] **Step 3: 서비스 3개 호출부 배선 (fileUrl→키, contentFileUrl→raw)**
...
그리고 `updateProduct` 안의 `anchor.update(...)` 호출(105행)과 두 `onSale.nextVersion(...)` 호출(119, 125행)에서 `request.content()` 뒤에 `newFileKey, request.contentFileUrl()`를 삽입한다. 예: in-place update:
...
patch nextVersion도 동일하게 `request.content(), newFileKey, request.contentFileUrl(),` 삽입(첫 인자 `false`).
...
git commit -m "feat: 상품 유형별 필드(file_url, content_file_url) 요청·엔티티·저장 배선"
```
...
**Interfaces:**
- Consumes: Task 1의 확장된 create/update/nextVersion 시그니처, `Product.getFileUrl()`/`getContentFileUrl()`
...
판매자가 자기 상품 상세에서 `fileUrl`(presigned) / `contentFileUrl`(원문)을 받도록 한다.
...
**Interfaces:**
- Consumes: `Product.getFileUrl()`, `Product.getContentFileUrl()`, `StorageClient.generatePresignedDownloadUrl(String)`
- Produces: `SellerProductDetailResponse`에 `fileUrl`, `contentFileUrl` 필드 추가

- [ ] **Step 1: 실패 테스트 작성**

`ProductSellerServiceTest.java`의 `GetMyProduct` nested class에 추가(헬퍼 `product(...)`는 PROMPT라 fileUrl/contentFileUrl이 null이므로, 필드를 세팅해 검증):
...
git commit -m "feat: 판매자 상품 상세 응답에 유형별 필드(fileUrl presigned, contentFileUrl) 노출"
```
...
- Modify: `docs/erd/schema.md` (Product 섹션에 `file_url`, `content_file_url`)
...
`content` 행 아래에 기존 컬럼 행 포맷과 동일하게 `file_url`(TEXT, NULL, "유형별 산출물 파일 스토리지 키(PPT/EXCEL)")와 `content_file_url`(TEXT, NULL, "외부 노션 링크 원문(NOTION)") 추가.

- [ ] **Step 4: api-spec/product.md에 유형별 필드 계약 추가**

판매자 생성/수정 요청 필드에 `fileUrl`, `contentFileUrl`(nullable) 및 `content`가 이제 유형별(PROMPT 필수)임을 명시하고, 유형별 매트릭스 표(설계 §3/§6)와 판매자 상세 응답의 `fileUrl`(presigned)/`contentFileUrl`(원문) 필드를 추가한다. 공개 상세에는 미노출임을 한 줄 명시.
...
- **타입 일관성**: create/update/nextVersion 시그니처의 `fileUrl, contentFileUrl` 위치가 세 Task에서 동일. `getFileUrl()/getContentFileUrl()`, `PRODUCT_TYPE_FIELD_MISMATCH` 이름 일치.
