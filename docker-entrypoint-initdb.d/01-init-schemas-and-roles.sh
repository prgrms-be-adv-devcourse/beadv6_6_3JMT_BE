...
# 배포 환경에서 서비스별 비밀번호를 필수로 주입받는다.
: "${USER_SERVICE_PASSWORD:?USER_SERVICE_PASSWORD is required}"
: "${PRODUCT_SERVICE_PASSWORD:?PRODUCT_SERVICE_PASSWORD is required}"
: "${ORDER_SERVICE_PASSWORD:?ORDER_SERVICE_PASSWORD is required}"
: "${PAYMENT_SERVICE_PASSWORD:?PAYMENT_SERVICE_PASSWORD is required}"
: "${SETTLEMENT_SERVICE_PASSWORD:?SETTLEMENT_SERVICE_PASSWORD is required}"
: "${NOTIFICATION_SERVICE_PASSWORD:?NOTIFICATION_SERVICE_PASSWORD is required}"
...
-- 계정으로 접속하는 Flyway는 만들지 못하므로 여기서 미리 만들어 둔다. 이미 있으면
-- V3의 CREATE EXTENSION IF NOT EXISTS는 권한 검사 없이 건너뛴다.
-- public이 아니라 product_service 스키마에 만든다 — 바로 아래에서 public 접근을
-- 막기 때문에, public에 있으면 product_service가 vector 타입을 쓰지 못한다.
CREATE EXTENSION IF NOT EXISTS vector SCHEMA product_service;
