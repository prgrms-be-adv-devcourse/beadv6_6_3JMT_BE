...
class OrderEventConsumerIntegrationTest extends AbstractIntegrationTest {
    ...
    void ORDER_REFUND_REQUESTED_수신_시_부분환불_처리() {
        ...
        Payment payment = Payment.create(orderId, userId, "pg-key-refund-1", "TOSS_PAYMENTS", "CARD", 10_000);
        ...
        send(orderId.toString(), orderRefundRequestedJson(orderId, UUID.randomUUID(), 4_000));
    ...
    void ORDER_REFUND_REQUESTED_외_이벤트타입은_무시() {
        ...
        UUID orderId = UUID.randomUUID();
        Payment payment = Payment.create(orderId, buyerId, "pg-key-refund-2", "TOSS_PAYMENTS", "CARD", 10_000);
        payment.markRequested(OffsetDateTime.now());
        payment.approve(10_000, "카드", "{}", "{}", OffsetDateTime.now());
        paymentJpaRepository.saveAndFlush(payment);
    ...
    private String orderRefundRequestedJson(UUID orderId, UUID refundRequestId, int refundAmount) {
