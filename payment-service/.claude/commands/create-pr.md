...
`/pre-pr-check` 커맨드와 동일하게 아래를 순서대로 실행한다.
...
- `git status --short` 출력이 있으면 "⚠️ 미커밋 변경이 있습니다."라고 경고하고 **즉시 중단**한다.
- `payment-service/` 하위에서 두 브랜치 간 겹치는 변경 파일이 있으면 "⚠️ 충돌 가능성 있는 파일: <목록>" 경고 후 계속 진행할지 사용자에게 확인한다.
...
- develop 대비 커밋이 0개이면 "PR에 포함할 커밋이 없습니다. 작업 후 다시 시도해 주세요."라고 안내하고 중단한다.
