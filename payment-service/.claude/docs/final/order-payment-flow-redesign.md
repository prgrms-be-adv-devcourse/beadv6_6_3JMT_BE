...
```json
{
  ...
  "payload": {
    "orderId": "UUID",
    "buyerId": "UUID",
    "orderNumber": "ORD-20260705-000001",
...
| 항목 | 값 |
|---|---|
| 토픽 / eventType | `payment.events` (작업 1 완료 기준) / `"payment.failed"` |
...
- `eventType == "ORDER_CREATED"`만 처리 → application 서비스 호출로 스냅샷 upsert. 나머지 타입은 ack 후 무시.
...
- `Payment.create()` 시그니처의 idempotencyKey 생성 규칙 변경(`pay-{orderId}` → paymentKey).
...
- 클래스 레벨 `@Transactional` 제거 → `confirm()` 메서드에 `@Transactional(noRollbackFor = BusinessException.class)` — 실패 Payment(FAILED) 저장이 롤백되지 않아야 실패 기록·이벤트가 남는다.
...
- 의존성은 이미 보유(grpc 1.80.0) — 서버 아티팩트는 grpc-netty-shaded에 포함되므로 빌드 변경 없음.

### 3. payment.failed 소비 (구 failure-event-publishing.md 흡수)

- `PaymentEventType`에 `PAYMENT_FAILED("payment.failed")` 추가, `PaymentEventConsumer`에 분기 라우팅 추가 (작업 1로 `payment.events` 단일 토픽 구독 전환 완료 상태).
...
- 기존 `ConfirmPaymentIntegrationTest` — amount 제거 반영 수정
