...
PG 결제 승인 실패(Toss confirm 호출 자체가 실패한 경우) 시 발행. `orderId`는 봉투 `aggregateId`와 동일한 값이지만, 다른 이벤트와 동일한 소비 패턴(payload에서 바로 필드 추출)을 유지하기 위해 중복 포함한다.
...
```json
{
  ...
  "payload": {
    "orderId": "660e8400-e29b-41d4-a716-446655440001",
    "failedAmount": 9900,
    "failedAt": "2026-06-15T19:02:00+09:00"
  }
}
```
...
|---|---|---|---|
| `orderId` | UUID | ✅ | 주문 ID |
| `failedAmount` | Int | ✅ | 시도했던 결제 금액 (`Payment.totalAmount`) |
| `failedAt` | ISO 8601 (KST) | ✅ | PG 승인 실패 처리 일시 |
...
`orderProductId`/`buyerId`는 order-service가 애초에 보내지 않는다 — payload에 존재하지 않는 필드다.
