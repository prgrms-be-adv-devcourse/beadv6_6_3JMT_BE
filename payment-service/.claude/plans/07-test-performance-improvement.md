...
전체 테스트 실행 시간이 약 55~115초로 과도하게 길다. 원인을 분석한 결과 아래 6가지 병목이 확인되었고, 단위 테스트로 이미 커버되는 불필요한 통합 테스트도 2개 확인되었다.
안전하게 적용 가능한 항목부터 순차 적용해 **전체 시간을 60% 이상 단축**하는 것이 목표다.
...
| `ConfirmPaymentIntegrationTest.java` | `extends AbstractIntegrationTest` 교체, 중복 컨테이너·프로퍼티 제거, `중복_결제_요청_시_409` 테스트 삭제 |
| `RefundPaymentIntegrationTest.java` | `extends AbstractIntegrationTest` 교체, `Thread.sleep` → Awaitility, Kafka 폴링 타임아웃 단축, `PAID_아닌_상태_환불_요청_시_400` 테스트 삭제 |
...
| 항목 | 개선 전 | 개선 후 |
|---|---|---|
| 통합 테스트 수 | 6개 | 4개 (불필요 2개 제거) |
| Kafka 폴링 대기 | 5~30초 | 1~5초 |
| Thread.sleep() | 6초 (2개) | 0.2~1초 (1개 → Awaitility) |
| 컨테이너 시작 횟수 | 6회 | 2회 (통합 1 + JPA 1) |
| Spring Context 로딩 | 4회 | 2회 |
| **전체 예상 시간** | **55~115초** | **15~25초** |
...
```
Phase 4 (불필요 테스트 삭제) → 테스트 실행 확인
→ Phase 1-1 (application-test.yml) → Phase 1-2 (Gradle 병렬) → 테스트 실행 확인
...
Phase 4를 먼저 적용하는 이유: 제거 대상 테스트(`PAID_아닌_상태_환불_요청_시_400`)에 Thread.sleep이 포함되어 있어, 초반 단계에서 제거하면 이후 각 단계의 실행 확인이 빨라진다.

각 단계 후 테스트를 실행해 회귀를 조기에 발견한다.

---

### Phase 4 — 불필요한 통합 테스트 제거

**목표**: 단위 테스트로 이미 검증되는 시나리오를 통합 테스트에서 제거해 실행 횟수와 컨테이너 의존성을 줄인다.

#### 통합 테스트 커버리지 분석

| 통합 테스트 메서드 | 단위 테스트 중복 여부 | 고유 검증 가치 | 판정 |
|---|---|---|---|
| `결제_승인_정상_플로우_DB_PAID_Kafka_메시지_수신` | 없음 | Kafka AFTER_COMMIT 전체 체인 | **유지** |
| `중복_결제_요청_시_409` | ServiceTest + ControllerTest 중복 | 없음 | **제거** |
| `환불_정상_플로우_DB_REFUNDED_Kafka_메시지_수신` | 없음 | Kafka AFTER_COMMIT 전체 체인 | **유지** |
| `PG_환불_실패_시_PAID_복원` | 없음 | RefundEventHandler AFTER_COMMIT 롤백 동작 | **유지** |
| `PAID_아닌_상태_환불_요청_시_400` | ServiceTest 중복 + Thread.sleep 포함 | 없음 | **제거** |
| `타인_결제_환불_시_403` | ServiceTest 부분 중복 | HTTP userId 인증(PAY006) 경로 | **유지** |

---

#### 4-1. `ConfirmPaymentIntegrationTest.중복_결제_요청_시_409()` 제거

**이미 커버하는 단위 테스트:**
- `ConfirmPaymentServiceTest.멱등성_키_중복_시_PAY002_예외()` — 멱등성 키 중복 비즈니스 로직 검증
- `PaymentControllerTest.중복_결제_시_409_PAY002()` — HTTP 409 응답 코드·에러 코드 매핑 검증

**제거 근거:**  
위 두 단위 테스트가 각각 비즈니스 계층과 프레젠테이션 계층을 완전히 커버한다. 통합 테스트가 추가로 검증하는 것은 실제 DB SELECT뿐이지만, 이는 단순 unique index 조회로 `PaymentJpaRepositoryTest`의 영속성 레이어 테스트 범위에 해당한다. Kafka 등 비동기 처리가 없으므로 통합 테스트의 고유 가치가 없다.

**변경:** `ConfirmPaymentIntegrationTest.java`에서 `중복_결제_요청_시_409()` 메서드 삭제.

---

#### 4-2. `RefundPaymentIntegrationTest.PAID_아닌_상태_환불_요청_시_400()` 제거

**이미 커버하는 단위 테스트:**
- `RefundPaymentServiceTest.PAID_아닌_상태_환불_시_PAY004_예외()` — PAY004 비즈니스 규칙 검증
- `PaymentControllerTest` — 에러 핸들러 HTTP 매핑 검증

**제거 근거:**  
이 테스트는 "이미 REFUNDED 상태에서 두 번째 환불 요청 → 400 PAY004" 시나리오를 검증하지만, `RefundPaymentServiceTest`의 `PAID_아닌_상태_환불_시_PAY004_예외()`가 동일한 코드 경로(`payment.refund()` 상태 검증)를 모킹 없이 도메인 객체로 직접 검증하고 있다. 통합 테스트가 추가하는 것은 실제 DB 상태 전환뿐이나, 이 상태 전환은 `환불_정상_플로우_DB_REFUNDED_Kafka_메시지_수신`에서 이미 end-to-end로 검증한다.

추가로 이 테스트는 `Thread.sleep(3_000)`을 포함해 **무조건 3초를 낭비**하는 주요 병목이다.

**변경:** `RefundPaymentIntegrationTest.java`에서 `PAID_아닌_상태_환불_요청_시_400()` 메서드 삭제.  
이 제거로 `Thread.sleep` 2개 중 1개가 함께 제거된다.

---

#### 제거 후 통합 테스트 구성

**ConfirmPaymentIntegrationTest** (1개):
- `결제_승인_정상_플로우_DB_PAID_Kafka_메시지_수신` — 승인 full flow + Kafka 검증

**RefundPaymentIntegrationTest** (3개):
- `환불_정상_플로우_DB_REFUNDED_Kafka_메시지_수신` — 환불 full flow + Kafka 검증
- `PG_환불_실패_시_PAID_복원` — AFTER_COMMIT 롤백 동작 검증
- `타인_결제_환불_시_403` — userId 기반 인가 HTTP 검증
