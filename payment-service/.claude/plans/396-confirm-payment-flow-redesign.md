...
**`Payment.fail()`의 상태 가드를 완화해 READY·REQUESTED 양쪽에서 FAILED로 전이할 수 있게 한다.** 도메인 규칙 자체는 READY에서도 실패 처리를 표현할 수 있어야 한다고 판단해 유지한다(예: 향후 다른 사전 검증 실패도 이 경로를 쓸 수 있음). 다만 아래 **[구현 중 변경]**대로, 금액 불일치는 이 경로를 타지 않기로 했다.

> **[구현 중 변경]** 금액 불일치는 Payment를 생성하지 않는다. Toss confirm을 호출한 적 없는 순수 입력 검증 실패를 "실패한 결제 시도"로 취급하면, ① `BLOCKING_STATUSES`의 `FAILED` 영구차단 정책과 얽혀 프론트 버그·가격 변경 등으로 실수로 틀린 금액을 보낸 사용자가 같은 주문을 영영 결제 못 하게 되고, ② Payment는 FAILED로 남는데 order-service는 이벤트를 받아도(또는 못 받아도) 실제로 결제 흐름이 시작된 적이 없어 상태 동기화의 의미가 없다. 그래서 금액 검증 실패는 Payment 레코드·PaymentFailedEvent 발행 없이 순수 400 응답으로만 처리하고, 같은 orderId로 올바른 금액이면 즉시 재시도 가능하게 바꿨다. `Payment.fail()`의 READY 허용 자체는 롤백하지 않았다(도메인 차원에서는 여전히 유효한 능력이라고 판단).
...
5. amount 검증 (command.amount == orderInfo.totalAmount)
   - 불일치: Payment 생성 없이 즉시 400 AMOUNT_MISMATCH 응답 (Toss 미호출, 이벤트 미발행 — 같은 orderId로 재시도 가능)
