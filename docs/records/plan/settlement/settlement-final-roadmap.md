...
| 항목 | 상태 | 상세 |
| --- | --- | --- |
| 어드민 모듈 분리 | 설계 확정 | [admin-module-separation.md](../../architecture/settlement/admin-module-separation.md) |
| 운영 배치 실행 CronJob 전환 | 구현 완료 (#364) | §2 |
| AI 정산 어시스턴트 | 3-2 구현 완료 (ai-service), 3-1·3-3 아이디어 | §3 |
| 정산 통계 · 대시보드 | 브레인스토밍 예정 | §4 |
| 지급 완료 Kafka 발행 | 확정 (설계 메모 있음) | — |
...
- 설계: [admin-module-separation.md](../../architecture/settlement/admin-module-separation.md)
- 운영 단일 진실(seller_settlement) 결정: [seller-settlement-separation.md](../trade-offs/settlement/seller-settlement-separation.md)
- 접근 방식 결정(직접 DB vs gRPC): [admin-data-access.md](../trade-offs/settlement/admin-data-access.md)
...
## 3. AI 정산 어시스턴트 — 3-2 구현 완료, 나머지 아이디어
...
### 3-2. 셀러 정산 문의 에이전트 (2순위 — 서비스 기능) — 구현 완료 (ai-service)
