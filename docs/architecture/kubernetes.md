...
| 자동 `push` | `develop` push | Config, Discovery, 비즈니스 서비스 6개, API Gateway의 변경된 이미지 |
...
1. 변경된 모듈만 감지한다. 공통 빌드 파일, `common-module`, Kubernetes 플랫폼·서비스·Gateway 매니페스트 또는 Kubernetes CD 워크플로가 바뀌면 모든 애플리케이션 이미지를 대상으로 한다. 따라서 최초 CD 전환 PR을 `develop`에 머지하면 애플리케이션 전체가 첫 배포 대상이 된다.
