...
현재 Docker Compose Gateway가 Large의 host 80을 사용하므로 Docker Gateway와 `hostNetwork` Controller는 동시에 실행할 수 없다. Kubernetes Gateway까지는 ClusterIP와 `kubectl port-forward`로 먼저 검증하고, 승인된 cutover 시점에만 Docker Gateway의 80 listener를 내린 뒤 Controller를 시작한다. 스모크 테스트에 실패하면 Controller를 중지하고 Docker Gateway를 다시 시작하는 것을 첫 rollback 경계로 삼는다. Controller 적용 전에 80, 443, 18080과 18081 listener 점유 여부를 모두 확인한다.
...
Ingress Controller는 Ingress, Service와 EndpointSlice를 감시해야 하므로 전용 ServiceAccount와 최소 RBAC를 사용하며 ServiceAccount token 자동 mount 비활성화 대상에서 제외한다.
...
1~8단계는 기존 Docker 트래픽을 유지한 채 수행한다. 9~10단계는 Large의 host 80 소유권을 Docker Gateway에서 F5 NGINX Ingress Controller로 넘기는 cutover 작업이므로 별도 승인을 받고 진행한다. 문서와 매니페스트 작성만으로 cutover를 수행하지 않는다.
