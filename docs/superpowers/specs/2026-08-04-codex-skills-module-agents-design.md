...
루트 `.claude/rules/**`는 특정 도구의 개인 메모리가 아니라 저장소의 팀 공통 컨벤션이므로 파일을 이동하거나 수정하지 않고 Codex 지침에서 참조한다. 루트 `AGENTS.md`가 변경 유형별 규칙을 라우팅하고, 커밋·브랜치·PR·전체 검증 스킬은 관련 Git 또는 검증 규칙을 직접 읽는다.

`git-convention.md`의 Claude 전용 `Co-Authored-By` 예시는 Codex 커밋에 적용하지 않는다. Codex는 Claude 이름을 기록하지 않으며 사용자의 명시적 요청 없이 다른 AI trailer로 바꾸지도 않는다. 그 밖의 공통 컨벤션은 그대로 적용한다.
