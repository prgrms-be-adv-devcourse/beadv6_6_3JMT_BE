...
- 8개 비즈니스 애플리케이션의 SLF4J 로그를 `application-logs-*`에 저장한다.
...
`admin-service`, `ai-service`, `settlement-service`, `notification-service`로 고정한다.
`notification-service`는 현재 Kubernetes 워크로드가 없으므로 구조화 로깅과 수집 allowlist만
준비하고, 별도 배포 작업으로 Pod가 생성된 시점부터 자동 수집한다. `config`, `discovery`,
API Gateway 일반 로그, init container와 인프라 로그는 제외한다.

## 2. 애플리케이션 로그 계약

8개 서비스는 Spring Boot의 Logstash structured console logging을 사용한다. SLF4J의
...
notification-service의 HTTP·SSE 연결 요청에는 같은 Servlet 계약을 적용하지만 Kafka consumer와
Redis Pub/Sub 처리에는 request ID 전파를 보장하지 않는다.

## 3. 수집 파이프라인

Fluent Bit은 Gateway용 input과 분리된 application input을 사용한다. application input은
`prompthub` namespace의 컨테이너 로그를 별도 DB로 tail하고 정확한 8개 container name만
...
활성화하며 Product-only canary 선택지는 제공하지 않는다. notification-service는 이번 변경에서
배포하지 않으며, 향후 Pod의 container name과 애플리케이션 이름이 `notification-service`로
배포되면 추가 ELK 변경 없이 수집된다.
...
- 8개 서비스 설정에 structured JSON과 올바른 service name이 존재함을 검증한다.
- 렌더된 Fluent Bit 설정이 8개 서비스만 허용하고 Gateway/init container를 제외함을 검증한다.
...
- 운영 확인에서는 현재 배포된 7개 서비스별 로그, 동일 `requestId`의 Gateway·HTTP 서비스 로그,
...
- config, discovery 로그 수집
- notification-service Kubernetes Deployment·Service·라우팅·Secret 구성
