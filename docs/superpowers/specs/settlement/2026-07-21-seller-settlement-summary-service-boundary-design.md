...
- `user-service/build.gradle`의 루트 `grpc/product` proto `sourceSets` 블록

루트 `grpc/product` 소스 경로는 Seller Settlement의 `GetSellerStats` 스텁 생성에만 사용되므로 함께
제거한다. user-service의 wishlist와 seller gRPC 서버는 `src/main/proto`의 별도 계약을 사용하므로
protobuf 플러그인, gRPC 서버·클라이언트 의존성과 공통 Product 채널 주소는 이번 작업에서 제거하지
않는다.
