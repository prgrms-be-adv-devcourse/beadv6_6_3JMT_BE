...
- `imageMinimumGCAge: 0s`는 kubelet 기본 최소 age `2m`으로 해석되고,
...
Expected: kubelet과 containerd가 `active`, 설정 파일의 최소·최대 age가 `0s`다. 최소 age
`0s`의 유효값은 kubelet 기본값 `2m`이다. high/low가 없으면 기본 85/80을 사용하는 현재
상태로 기록한다.
...
```text
두 노드 kubelet 이미지 GC: high 60 / low 50 / min age 설정 0s(유효값 2m) / max age 0s(비활성)
