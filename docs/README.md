...
│   ├── plan/              # 기획·계획(plans)·설계(specs)·로드맵·백로그 — 도메인별 폴더(order/product/settlement/user), 도메인에 속하지 않는 인프라·횡단 관심사는 infra/
...
> "어떻게·왜 그렇게 했나"를 담습니다. 기획·계획·설계·완료 산출물은 모두 `records/plan/`에
> 도메인별 폴더로 보관합니다(예: `records/plan/order/`).
>
...
2. 도메인 폴더로 분리하기 모호한 경우 파일명 앞에 서비스명 prefix를 붙여 작성하세요. (예: `docs/records/plan/user/user-기획문서.md`)
