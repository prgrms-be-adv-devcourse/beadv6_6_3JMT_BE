# Gateway 주문 상품 단건 환불 진입점 요청

## 요청 사항

- Gateway는 이미 `/api/v2/orders/**`를 Order Service로 라우팅한다. Order Service의 공개 단건 환불 진입점은 `POST /api/v2/orders/{orderId}/refund`이며, 이 경로를 같은 라우팅 정책으로 제공해야 한다.
...
- 요청 body는 `paymentId`, `orderProductId` 한 건이며 Gateway에서 목록 형태로 변환하지 않는다.
