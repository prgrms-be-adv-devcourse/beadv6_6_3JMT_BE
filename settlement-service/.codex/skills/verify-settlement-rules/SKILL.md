...
4. commit message의 `Co-Authored-By` trailer를 확인한다. 이슈·PR 초안이 입력에 포함되거나 검증 대상이면 현재 checkout의 `.github/PULL_REQUEST_TEMPLATE.md`와 관련 `.github/ISSUE_TEMPLATE/` 파일을 완전히 읽고 번호 유형, 담백한 문체, 실제 메타데이터를 확인한다.
