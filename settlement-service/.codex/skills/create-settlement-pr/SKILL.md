...
10. 승인 후에만 push와 `gh pr create --body-file`을 실행한다. 기본 생성 명령에는 `--draft`를 넣지 않는다.
11. 생성 직후 `gh pr view --json isDraft,labels,assignees,reviewRequests`로 상태와 메타데이터를 검증한다. 누락된 기본값은 보고만 하지 말고 `gh pr ready` 또는 `gh pr edit`로 적용한 뒤 다시 검증한다.
