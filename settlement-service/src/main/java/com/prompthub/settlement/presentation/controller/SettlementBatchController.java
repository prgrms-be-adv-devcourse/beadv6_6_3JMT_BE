...
@Tag(name = "Settlement Batch", description = "로컬 수동 검증용 정산 배치잡 API")
public class SettlementBatchController {
	...
	@ApiResponses({
		...
		@ApiResponse(responseCode = "400", description = "요청 값 또는 실행자 ID 오류",
	...
	@Operation(summary = "정산 배치잡 상태 조회",
		description = "로컬에서 수동 실행한 정산 배치 잡의 진행 또는 완료 상태를 조회합니다.")
	@ApiResponses({
