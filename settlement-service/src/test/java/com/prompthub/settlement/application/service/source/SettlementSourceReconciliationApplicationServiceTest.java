...
class SettlementSourceReconciliationApplicationServiceTest {
    ...
    private OrderSettlementClient orderSettlementClient;
    ...
    @BeforeEach
    void setUp() {
        orderSettlementClient = mock(OrderSettlementClient.class);
    ...
    void reconcile_matchingAggregates_returnsMatched() {
        given(orderSettlementClient.fetchSettleableLines(PERIOD)).willReturn(List.of(
    ...
    void reconcile_paidAmountMismatch_returnsMismatch() {
        given(orderSettlementClient.fetchSettleableLines(PERIOD))
    ...
    void reconcile_samePeriodTwice_returnsSameResult() {
        ...
        given(orderSettlementClient.fetchSettleableLines(PERIOD)).willReturn(orderLines);
        given(settlementSourceAggregateRepository.aggregate(PERIOD))
        ...
        then(orderSettlementClient).should(times(2)).fetchSettleableLines(PERIOD);
