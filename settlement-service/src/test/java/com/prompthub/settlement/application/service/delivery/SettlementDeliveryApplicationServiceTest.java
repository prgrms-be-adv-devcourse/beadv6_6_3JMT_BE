...
class SettlementDeliveryApplicationServiceTest {
    ...
    @Mock SellerSettlementClient registration;
    ...
    void UNAVAILABLE은_두번_재시도한_뒤_대사_완료한다() {
        ...
        given(registration.register(command))
                .willThrow(clientFailure("UNAVAILABLE", true))
                .willThrow(clientFailure("UNAVAILABLE", true))
                .willReturn(org.mockito.Mockito.mock(
                        com.prompthub.settlement.application.dto.delivery
    ...
    void 최종_실패해도_다음_Delivery를_처리한다() {
        ...
        given(registration.register(command))
                .willThrow(clientFailure("INVALID_ARGUMENT", false))
                .willReturn(org.mockito.Mockito.mock(
                        com.prompthub.settlement.application.dto.delivery
                                .SellerSettlementStoredSnapshot.class));
        given(reconciler.compare(any(), any()))
    ...
    void retriesFailedDeliveryWithThreeAdditionalAttempts() {
        ...
        given(registration.register(command))
                .willThrow(clientFailure("UNAVAILABLE", true))
                .willThrow(clientFailure("DEADLINE_EXCEEDED", true))
                .willReturn(org.mockito.Mockito.mock(
                        com.prompthub.settlement.application.dto.delivery
                                .SellerSettlementStoredSnapshot.class));
        given(reconciler.compare(any(), any()))
    ...
    private SellerSettlementClientException clientFailure(
            String code,
            boolean retryable) {
        return new SellerSettlementClientException(
                new SellerSettlementClientFailure(code, retryable),
                "gRPC " + code,
                null);
    }
}
