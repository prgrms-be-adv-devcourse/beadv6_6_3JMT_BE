...
| 데이터 | 조회 시점 | 호출 대상 |
|--------|----------|----------|
| 판매자 이름(상점명) — 다건 | 정산 목록 조회 시(판매자 여러 명) | User: `POST /internal/sellers/batch` (§2-1) |
...
> **한 번에** `POST /internal/sellers/batch` 로 조회해 `sellerId → sellerName` 으로 매핑한다.
...
| 판매자 이름(다건) | 동기 조회 — User `POST /internal/sellers/batch` (정산 목록용, N+1 회피). 신설 요청 필요 |
