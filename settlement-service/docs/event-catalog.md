...
| `order-events` | `ORDER_REFUNDED`(예정) | Order | 주문상품 환불 시 `settlement_source_line`(REFUND) 적재 | `orderProductId` + REFUND | **미발행 — Order 팀에 발행 요청 예정** |

> Order 는 현재 `ORDER_PAID` 만 발행한다. 환불(`ORDER_REFUNDED`)은 아직 발행하지 않으며, Settlement 가
> Order 팀에 발행을 요청할 예정이다. 그때 `OrderEventType`·컨슈머 분기에 환불을 추가한다.
...
| `ORDER_REFUNDED` 수신 | 미발행 — Order 팀에 발행 요청 예정 |
