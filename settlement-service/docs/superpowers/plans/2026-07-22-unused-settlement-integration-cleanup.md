...
**Goal:** user-service와 settlement-service에 남은 호출자 없는 gRPC 서버 묶음과 미사용 이벤트 타입을 제거하면서 Kafka seed와 Order gRPC 동작을 보존한다.

**Architecture:** user-service의 마지막 gRPC 서버 어댑터·로컬 proto·서버 전용 설정을 하나의 삭제 단위로 걷어내고 REST Seller 조회 유스케이스는 유지한다. settlement-service에서는 참조 없는 이벤트 enum만 제거하고 현재 구조를 설명하는 모듈 내부 문서를 갱신한다. admin settlement는 네트워크 연동이 없으므로 변경하지 않고 회귀 테스트만 수행한다.
...
- 과거 `docs/superpowers/specs/`, `docs/superpowers/plans/`와 user-service 날짜 기반 설계 문서는 소급 수정하지 않는다.
...
### Task 2: settlement-service의 미사용 이벤트 타입과 현재 통신 문서 정리

**Files:**
- Delete: `settlement-service/src/main/java/com/prompthub/settlement/application/event/SettlementEventType.java`
...
**Interfaces:**
- Consumes: `JsonOutboxEventAppender.appendSettlementCreated(UUID, SettlementCreatedEvent)`가 현재 `SETTLEMENT_CREATED` 문자열로 만드는 `EventMessage` 계약.
- Produces: `SettlementEventType` 없이 동일한 `SETTLEMENT_CREATED` Outbox payload를 발행하는 settlement-service. `OrderSettlementQueryClient`와 `SettlementEventPublisher.publish(String, UUID, String)`는 변경하지 않는다.

- [ ] **Step 1: 미사용 enum과 보존할 Kafka·Order gRPC 경로의 기준선을 확인한다**
...
Expected: enum 선언 파일과 `kafka-messaging-design.md` 구현 트리만 출력된다. Java 사용처는 선언 자체 외에 없어야 한다.
...
- [ ] **Step 2: 참조 없는 SettlementEventType을 삭제한다**

다음 파일을 전체 삭제한다.

```text
settlement-service/src/main/java/com/prompthub/settlement/application/event/SettlementEventType.java
```

`SettlementCreatedEvent`, `JsonOutboxEventAppender`와 `KafkaSettlementEventPublisher`는 수정하지 않는다.

- [ ] **Step 3: Kafka 구현 트리에서 삭제한 enum만 제거한다**
...
- [ ] **Step 4: 정산 내부통신 토폴로지를 현재 코드에 맞춘다**
...
- [ ] **Step 5: 연동 카탈로그의 Seller gRPC 계약을 REST 화면 조합 경계로 바꾼다**
...
- [ ] **Step 6: 삭제 후 Kafka Outbox와 Order gRPC 회귀를 검증한다**
...
- [ ] **Step 7: settlement-service 삭제와 문서 현행화를 커밋한다**

```bash
git add -u settlement-service/src/main/java/com/prompthub/settlement/application/event/SettlementEventType.java
...
git commit -m "refactor: 정산 연동 미사용 타입 정리"
```

Expected: settlement-service 코드와 현재 아키텍처 문서만 포함한 커밋이 생성된다.
...
**Interfaces:**
- Consumes: Task 1의 gRPC 없는 user-service 빌드와 Task 2의 enum 없는 settlement-service 빌드.
...
Expected: 두 명령 모두 출력 없음, 종료 코드 1.
