...
알림 발행은 추후·파이널. 어드민 모듈(admin-service)은 gRPC가 아니라 DB를 직접 바라보므로
rpc 계약이 없다. 운영 조회·상태변경은 `seller_settlement`(유저 DB)가 담당하고, 운영 배치 실행은
Kubernetes CronJob 전환을 목표로 한다. 연동 카탈로그 §4.)

| 문서 | 내용 |
| --- | --- |
| [Spring Batch 정산 흐름 원본](settlement-spring-batch-flow.excalidraw) · [PNG](settlement-spring-batch-flow.png) | 실행 트리거부터 6개 Step, order gRPC 원천 적재, 정산 DB, Outbox/Kafka 발행, 실패 처리까지 한 장으로 정리한 다이어그램. Excalidraw 원본은 요소별로 편집할 수 있다. |
| [settlement-internal-comm-topology.md](settlement-internal-comm-topology.md) | **정산 도메인 3모듈(정산 본체·셀러 정산·어드민 정산)의 현재 통신 상태 현황판.** 어느 모듈이 무엇을 발행/구독/서버/클라이언트로 구현·비활성·대기 중인지 매트릭스로 정리. **"지금 어떻게 통신하나"** 에 해당. |
| [integration-catalog.md](integration-catalog.md) | 외부와 주고받는 데이터 **계약 상세**. 원천 데이터(매출/환불) gRPC pull 과 참고 데이터(판매자 정보·상품 수) 동기 조회를 proto·멱등키 단위로 정리. **"무엇을"** 에 해당. (분산 전 정산 본체 기준으로 쓰여 §2 는 현재 셀러 정산으로 이관 — 각 절 노트·토폴로지 문서 참조.) |
