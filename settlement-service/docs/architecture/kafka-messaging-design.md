...
| 요소 | 참고 구현 | 정산에 가져올지 |
|------|----------|----------------|
| AFTER_COMMIT 직접 발행 | payment-service | 기본으로 가져와 `SETTLEMENT_CREATED` 에 적용(§3-1) |
...
infrastructure/messaging/kafka/
  ...
  producer/
    ...
    OutboxRelay.java                       ← (정합성 critical 흐름 도입 시) @Scheduled 폴링 발행  (추후)
```
...
- **적용 대상 — 유실돼도 치명적이지 않은 알림성/seed 이벤트.**
  - **`SETTLEMENT_CREATED` (✅ 구현됨).** 배치가 정산을 계산·생성한 뒤 셀러 정산(user-service
    `sellersettlement`) 초기행 seed 용으로 발행. 트리거: 정산 생성 트랜잭션 커밋 후(AFTER_COMMIT).
    유실 대비는 셀러 컨슈머 멱등(`settlementId` 유니크) + 배치 재실행으로 흡수한다. (아웃박스 flush 는
    아직 미구현 — `trade-offs/seller-settlement-separation.md` "seed 경로".)
...
3. **(추후·파이널) 발행(payout.completed)** — 지급 완료 알림. 같은 AFTER_COMMIT 패턴으로 추가.
