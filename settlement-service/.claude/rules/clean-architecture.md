...
└── settlement                      ← 기능(도메인) 패키지
    ...
    │   │   ├── runner              ← Kubernetes CronJob one-shot 실행 어댑터(SettlementCronJobRunner)
...
- Job / Step / Reader / Processor / Writer / Tasklet / Listener 구성은 모두 `infrastructure/batch` 아래,
  **역할별 하위 패키지로 분리**한다. (`config` · `reader` · `processor` · `writer` · `tasklet` · `listener` · `launcher` · `runner` · `model`)
