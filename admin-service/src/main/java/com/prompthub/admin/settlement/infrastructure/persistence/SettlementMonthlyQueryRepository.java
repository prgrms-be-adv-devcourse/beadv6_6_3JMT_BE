...
public class SettlementMonthlyQueryRepository {
	...
	public MonthlyPage findMonthlyPage(
			...
			int page,
			int size) {
		return findMonthlyPage(null, status, settlementMonth, page, size);
	}
	...
	public List<Settlement> findWeeklySettlements(
			UUID sellerId, YearMonth settlementMonth) {
	...
	private MonthlyPage findMonthlyPage(
			UUID sellerId,
			SettlementDisplayStatus status,
			YearMonth settlementMonth,
			int page,
		...
		String whereClause = createWhereClause(
				sellerId, status, settlementMonth, parameters);
	...
	private String createWhereClause(
		...
		if (sellerId != null) {
			...
			conditions.add("s.seller_id = :sellerId");
		}
		if (settlementMonth != null) {
			...
			conditions.add("s.period_start >= :periodStart");
			conditions.add("s.period_start < :periodEnd");
		}
		if (status != null) {
	...
	public record MonthlyKey(UUID sellerId, YearMonth settlementMonth) {
	}
	...
	public record MonthlyStatusCount(
			MonthlyKey key,
			SettlementDisplayStatus status,
			long count) {
	}

	public record MonthlyPage(List<MonthlyAggregate> content, long totalElements) {
	}
}
