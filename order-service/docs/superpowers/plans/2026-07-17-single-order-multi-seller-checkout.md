...
- schema 변경은 V2의 seller 소유권 이동과 V3의 정산 조회 index 추가로 확정했다. PostgreSQL 18.4-alpine 일회용 환경에서 sample 2 Order/3 OrderProduct를 포함한 V1 → V2 → V3 순차 적용을 검증했다.
- V2 뒤 `order.seller_id` 부재, `order_product.seller_id NOT NULL`과 seller backfill을 확인했다. V3 뒤 `idx_order_product_order_id`, `idx_order_product_refunded_at` 및 V2의 `idx_order_product_seller_created_at`을 catalog에서 확인했다.
...
| `./gradlew :order-service:build --rerun-tasks` | `BUILD SUCCESSFUL in 1m 26s`; 16 tasks executed, 기존 checkstyle warning은 non-failing |
...
- [ ] `V1__baseline.sql`은 과거 baseline이므로 수정하지 않는다. 신규 DB도 V1 → V2 → V3 순서로 최종 schema에 도달하게 한다.
...
### 9.1 Disposable PostgreSQL에서 V1 → V2 → V3 검증

- [ ] 전용 test container가 없는지 확인한다.

```bash
docker ps --all --filter name=order-service-migration-test
```

Expected: header 외 container 행 없음.

- [ ] 전용 test container를 시작한다.

```bash
docker run --detach \
  --name order-service-migration-test \
  --env POSTGRES_PASSWORD=order_test \
  --env POSTGRES_DB=order_test \
  --publish 55432:5432 \
  postgres:18.4-alpine
```

Expected: container ID 출력.

- [ ] DB 준비 상태를 확인한다. 준비 전이면 같은 명령을 최대 30초 동안 다시 실행한다.

```bash
docker exec order-service-migration-test pg_isready -U postgres -d order_test
```
...
```bash
docker exec -i order-service-migration-test psql -v ON_ERROR_STOP=1 -U postgres -d order_test \
  < order-service/src/main/resources/db/migration/V1__baseline.sql
```

Expected: 모든 DDL 성공.

- [ ] 서로 다른 기존 Order 2건과 OrderProduct 3건 sample을 넣는다.

```bash
docker exec order-service-migration-test psql -v ON_ERROR_STOP=1 -U postgres -d order_test -c \
  "INSERT INTO \"order\" (id, created_at, updated_at, buyer_id, seller_id, order_number, order_status, total_order_amount) VALUES ('00000000-0000-0000-0000-000000000501', now(), now(), '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000101', 'ORD-MIG-1', 'CREATED', 3000), ('00000000-0000-0000-0000-000000000502', now(), now(), '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000102', 'ORD-MIG-2', 'CREATED', 3000); INSERT INTO order_product (id, created_at, updated_at, downloaded, order_product_status, product_amount_snapshot, product_id, product_title_snapshot, order_id) VALUES ('00000000-0000-0000-0000-000000000601', now(), now(), false, 'PENDING', 1000, '00000000-0000-0000-0000-000000000201', '상품 A1', '00000000-0000-0000-0000-000000000501'), ('00000000-0000-0000-0000-000000000602', now(), now(), false, 'PENDING', 2000, '00000000-0000-0000-0000-000000000202', '상품 A2', '00000000-0000-0000-0000-000000000501'), ('00000000-0000-0000-0000-000000000603', now(), now(), false, 'PENDING', 3000, '00000000-0000-0000-0000-000000000203', '상품 B1', '00000000-0000-0000-0000-000000000502');"
```

Expected: `INSERT 0 2`와 `INSERT 0 3`.

- [ ] V2를 적용한다.

```bash
docker exec -i order-service-migration-test psql -v ON_ERROR_STOP=1 -U postgres -d order_test \
  < order-service/src/main/resources/db/migration/V2__move_seller_id_to_order_product.sql
```

Expected: migration 성공.

- [ ] seller 역채움 결과를 조회한다.

```bash
docker exec order-service-migration-test psql -U postgres -d order_test -Atc \
  "select id, seller_id from order_product order by id;"
```
...
00000000-0000-0000-0000-000000000603|00000000-0000-0000-0000-000000000102
```

- [ ] 부모 column 제거와 child NOT NULL을 확인한다.

```bash
docker exec order-service-migration-test psql -U postgres -d order_test -Atc \
  "select table_name, column_name, is_nullable from information_schema.columns where (table_name='order' and column_name='seller_id') or (table_name='order_product' and column_name='seller_id') order by table_name;"
```

Expected: `order_product|seller_id|NO` 한 줄만 출력.

- [ ] V3를 V2 다음에 적용한다.

```bash
docker exec -i order-service-migration-test psql -v ON_ERROR_STOP=1 -U postgres -d order_test \
  < order-service/src/main/resources/db/migration/V3__add_settlement_query_indexes.sql
```

Expected: `CREATE INDEX` 2회 성공.

- [ ] 최종 index catalog를 확인한다.

```bash
docker exec order-service-migration-test psql -U postgres -d order_test -Atc \
  "select indexname from pg_indexes where tablename='order_product' and indexname in ('idx_order_product_order_id', 'idx_order_product_refunded_at', 'idx_order_product_seller_created_at') order by indexname;"
```

Expected: V2의 seller/created index와 V3의 order ID/refundedAt index, 총 3개가 출력된다.

- [ ] 검증 container를 제거한다.

```bash
docker rm --force order-service-migration-test
```

Expected: `order-service-migration-test` 출력. 이 container는 volume을 연결하지 않은 disposable 검증 환경이다.
...
- PostgreSQL 18.4-alpine에서 V1 → sample → V2 → V3가 모두 exit 0이었다.
...
git commit -m "docs: order-service 단일 주문 다중 판매자 계약 문서화"
```
...
2. DB backup 또는 즉시 복원 가능한 snapshot을 만든다.
3. 주문 생성 트래픽을 중지한다.
4. 실행 중인 구 Order Service writer를 모두 중지한다.
5. Flyway V2를 적용하고 null count 0, child NOT NULL, parent column 제거를 확인한다.
6. Flyway V3를 적용하고 정산 조회 index 2개를 catalog에서 확인한다.
7. 새 Order Service를 배포한다.
8. 4상품·3판매자 Checkout smoke test로 Order 1, OrderProduct 4, Outbox 1을 확인한다.
9. `ORDER_CREATED`가 Payment snapshot 1건을 만들고 결제 승인 요청이 Payment 1건만 만드는지 확인한다.
10. 승인 후 Order COMPLETED, 상품 4건 PAID, `ORDER_PAID` 1건, Redis 만료 제거를 확인한다.
11. seller A 상품 1개를 환불해 PARTIAL_REFUNDED와 `ORDER_REFUND.products=1`을 확인한다.
12. 마지막 상품까지 환불해 ALL_REFUNDED를 확인한다.
13. 관리자 응답 seller summary 3개와 대상 월 정산 PAID/REFUND line seller ID를 확인한다.
14. 이상이 없으면 주문 생성 트래픽을 재개한다.

Application만 이전 버전으로 rollback할 수 없다. rollback은 DB snapshot 복원 또는 seller column 역 migration과 구 application을 함께 적용해야 한다.
...
- [x] PostgreSQL V1 → V2 → V3 migration rehearsal이 성공한다.
