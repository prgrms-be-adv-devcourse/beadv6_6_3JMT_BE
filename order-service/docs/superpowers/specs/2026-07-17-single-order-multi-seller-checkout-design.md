...
schema 변경은 `V2__move_seller_id_to_order_product.sql`과 `V3__add_settlement_query_indexes.sql`로 나눈다. V2 적용 순서는 다음과 같다.
...
기존 Order가 한 판매자만 소유하므로 역채움 값은 모든 기존 OrderProduct에 동일하게 복사된다. migration은 PostgreSQL transaction 안에서 V1 → V2 → V3 순으로 실행하며, 적용 중 주문 생성 트래픽을 중지한다.
...
- PostgreSQL 18.4-alpine disposable DB에서 V1 → sample 2 Order/3 OrderProduct → V2 → V3를 순서대로 적용했다. seller backfill, parent seller column 제거, child NOT NULL, 최종 index 3개를 확인했고 container를 제거했다.
- `./gradlew :order-service:test --rerun-tasks`: `BUILD SUCCESSFUL in 1m 25s`, 428 tests, skip/failure/error 0.
- `./gradlew :order-service:build --rerun-tasks`: sandbox wrapper-lock 실패 1회 후 승인된 동일 명령은 `BUILD SUCCESSFUL in 1m 26s`, 16 tasks executed. 기존 checkstyle/deprecation 및 Embedded Kafka 종료 warning은 non-failing이다.
...
1. Order·Payment Kafka 계약 fixture를 먼저 확정한다.
2. 주문 생성 트래픽을 일시 중지한다.
3. Flyway V2 migration으로 seller ID를 역채움하고 column을 이동한다.
4. Flyway V3 migration으로 정산 조회 index 두 개를 추가하고 catalog를 확인한다.
5. 새 Order Service를 배포한다.
6. 상품 4개·판매자 3명 Checkout smoke test를 수행한다.
7. Order 1건, OrderProduct 4건, Outbox 1건을 확인한다.
8. 결제를 승인하고 Payment 1건, Order COMPLETED, 상품 4건 PAID를 확인한다.
9. 서로 다른 판매자의 상품을 한 건씩 환불해 상태와 정산 line을 확인한다.
10. 이상이 없으면 주문 생성 트래픽을 재개한다.

schema column을 제거하므로 application만 이전 버전으로 rollback할 수 없다. rollback이 필요하면 DB backup 복원 또는 역 migration과 이전 application을 함께 적용해야 한다.
...
- PostgreSQL V1 → V2 → V3 rehearsal에서 seller backfill, NOT NULL과 최종 정산 index를 확인한다.
