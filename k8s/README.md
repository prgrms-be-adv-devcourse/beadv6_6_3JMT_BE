...
kubectl kustomize k8s/overlays/learning
```
...
스크립트는 모든 패키지가 렌더링되는지, `latest` 이미지나 미정 placeholder가 없는지, 실제 Secret이 base에 포함되지 않는지를 확인한다. Ingress Controller의 host network·이미지 digest·필수 인자와 learning overlay의 Gateway Ingress도 검사하고 NodePort·LoadBalancer 회귀를 차단한다.
...
하나라도 `NotFound`면 platform·services·gateway 또는 전체 learning overlay를 실제 적용하지 않는다.
...
kubectl apply --dry-run=client -k k8s/overlays/learning
```
...
kubectl apply --server-side --dry-run=server -k k8s/overlays/learning
```
...
6. F5 NGINX Ingress Controller와 learning Ingress를 적용한다.
