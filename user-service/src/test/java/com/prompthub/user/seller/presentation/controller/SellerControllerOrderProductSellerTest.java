...
class SellerControllerOrderProductSellerTest {
    ...
    @Test
    void getOrderProductSeller_정상_조회_200() throws Exception {
        ...
        mockMvc.perform(get("/api/v2/users/order-product").param("sellerId", SELLER_ID.toString()))
    ...
    @Test
    void getOrderProductSeller_profileImageUrl_미등록시_null() throws Exception {
        given(sellerQueryUseCase.findSeller(SELLER_ID.toString()))
    ...
    @Test
    void getOrderProductSeller_sellerId_누락_400() throws Exception {
        mockMvc.perform(get("/api/v2/users/order-product"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.code").value("V001"));
    }

    @Test
    void getOrderProductSeller_sellerId_UUID_형식_아님_400() throws Exception {
        mockMvc.perform(get("/api/v2/users/order-product").param("sellerId", "not-a-uuid"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.code").value("V001"));
    }

    @Test
    void getOrderProductSeller_존재하지_않는_sellerId_404() throws Exception {
        given(sellerQueryUseCase.findSeller(SELLER_ID.toString()))
