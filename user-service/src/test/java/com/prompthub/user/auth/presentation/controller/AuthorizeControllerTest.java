...
class AuthorizeControllerTest {
    ...
    @Test
    void authorize_epoch_일치_시_200_status와_role_반환() throws Exception {
        given(authorizeUseCase.authorize(USER_ID, 3L))
                .willReturn(new AuthorizeResult(UserStatus.ACTIVE, UserRole.BUYER));

        mockMvc.perform(get("/internal/authorize/{userId}?epoch=3", USER_ID))
    ...
    void authorize_사용자_없으면_404() throws Exception {
        given(authorizeUseCase.authorize(USER_ID, 3L)).willThrow(new UserNotFoundException());
    ...
    @Test
    void authorize_epoch_파라미터_없으면_401() throws Exception {
        given(authorizeUseCase.authorize(USER_ID, null)).willThrow(new SessionInvalidatedException());

        mockMvc.perform(get("/internal/authorize/{userId}", USER_ID))
                .andExpect(status().isUnauthorized())
                .andExpect(jsonPath("$.code").value("A013"));
    }

    @Test
    void authorize_epoch_불일치면_401() throws Exception {
        given(authorizeUseCase.authorize(USER_ID, 1L)).willThrow(new SessionInvalidatedException());

        mockMvc.perform(get("/internal/authorize/{userId}?epoch=1", USER_ID))
                .andExpect(status().isUnauthorized())
                .andExpect(jsonPath("$.code").value("A013"));
    }
}
