...
- [x] `OAuthLoginResult`에 `loginStatus` 기반 결과 구분을 추가한다.
- [x] 정상 로그인 결과와 재가입 필요 결과가 잘못 섞이지 않도록 구분 가능한 응답 타입으로 만든다.
- [x] 정상 로그인 응답에는 기존 필드를 유지해 기존 프론트엔드 호환성을 보존한다.
- [x] `REJOIN_REQUIRED` 응답에는 서비스 토큰이 직렬화되지 않게 한다.
- [x] Swagger에 상태별 응답 예시와 필드 조건을 추가한다.

### 8.2 OAuth 로그인 서비스

- [x] 기존 OAuth 연동으로 사용자를 조회한 뒤 `UserStatus`를 검사한다.
- [x] `ACTIVE` 사용자는 기존 로그인 흐름을 유지한다.
- [x] 신규 사용자는 기존 자동 회원가입 흐름을 유지한다.
- [x] `WITHDRAWN` 사용자는 재가입 토큰만 생성하고 AT/RT 발급 로직을 실행하지 않는다.
- [x] `BLOCKED` 사용자는 `AUTH_FORBIDDEN(A004)`로 거부하고 어떤 토큰도 발급하지 않는다.
- [x] 로그인과 재가입에서 중복되는 AT/RT 발급 로직을 하나의 내부 컴포넌트로 분리한다.

### 8.3 재가입 토큰

- [x] 재가입 토큰 생성·저장·소비를 위한 포트와 Redis 어댑터를 추가한다.
- [x] TTL을 5분으로 적용한다.
- [x] Redis에는 토큰 해시와 복구 대상 사용자 ID만 저장한다.
- [x] 소비 연산은 동시 요청에서도 하나만 성공하도록 원자적으로 구현한다.
- [x] Redis 장애 시 재가입을 허용하지 않는 fail-closed 정책을 적용한다.

### 8.4 재가입 유스케이스

- [x] `POST /api/v2/auth/rejoin` 컨트롤러와 유스케이스를 추가한다.
- [x] 요청 DTO에서 빈 토큰을 검증한다.
- [x] 토큰을 원자적으로 소비하고 대상 사용자를 조회한다.
- [x] 사용자 상태가 여전히 `WITHDRAWN`인지 확인한다.
- [x] `User.activate()`로 상태만 `ACTIVE`로 변경한다.
- [x] 사용자 ID, 이메일, 역할 및 연관 데이터를 변경하지 않는다.
- [x] 새 Refresh Token과 Access Token을 발급한다.
- [x] 인가 캐시에 `ACTIVE` 상태와 기존 대표 역할을 저장한다.
- [x] 결과를 `COMPLETED`, `isNewUser=false`로 반환한다.

### 8.5 오류·API 문서

- [x] `AUTH_REJOIN_TOKEN_INVALID(A014)` 오류 코드를 추가한다.
- [x] `docs/error-codes.md`에 오류 코드를 반영한다.
- [x] `docs/api-spec/auth.md`에 로그인 분기와 재가입 API를 반영한다.
- [x] OpenAPI 애너테이션을 갱신한다.
- [x] API 변경 내용을 프론트엔드 설계 문서에 반영한다.

## 9. 테스트

### 9.1 OAuth 로그인

- [x] 신규 사용자는 `COMPLETED`, `isNewUser=true`와 AT/RT를 받는다.
- [x] 기존 `ACTIVE` 사용자는 `COMPLETED`, `isNewUser=false`와 AT/RT를 받는다.
- [x] `WITHDRAWN` 사용자는 `REJOIN_REQUIRED`와 재가입 토큰만 받는다.
- [x] `WITHDRAWN` 로그인 단계에서 Refresh Token 저장소가 호출되지 않는다.
- [x] `BLOCKED` 사용자는 403을 받고 어떤 토큰도 받지 않는다.

### 9.2 재가입

- [x] 유효한 재가입 토큰으로 상태가 `ACTIVE`로 변경된다.
- [x] 재가입 전후 사용자 ID, 이메일 및 역할이 동일하다.
- [x] 재가입 성공 시 새 AT/RT가 발급되고 ACTIVE 인가 캐시가 저장된다.
- [x] 만료, 변조, 재사용 토큰은 모두 `A014`로 거부된다.
- [x] 동시에 같은 토큰을 사용하면 한 요청만 성공한다.
- [x] 재가입 처리 중 상태가 이미 변경됐으면 `A014`로 거부한다.
- [x] Redis 장애 시 재가입이 실패하고 계정이 활성화되지 않는다.
- [x] Controller 테스트에서 상태별 JSON 필드 포함·미포함을 검증한다.
