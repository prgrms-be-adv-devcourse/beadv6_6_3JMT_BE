...
- **다른 모듈은 읽지 않는다.** 코드·룰·CLAUDE.md 포함, 다른 서비스 모듈은 열람 대상이 아니다
  (루트 `CLAUDE.md` 전역 규칙). 다른 모듈 관련 맥락이 필요하면 직접 찾지 말고 사용자에게 물어본다.
  다른 모듈의 코드를 바꿔야 할 일이 생기면 직접 고치지 말고 사용자에게 알린다.
