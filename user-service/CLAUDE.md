...
**이 모듈(`user-service/`) 외부는 절대 수정하지 않는다.**

- `../common-module/`, `../apigateway/`, `../config/`, `../discovery/`, `../order-service/` 등 다른 모듈의 파일을 생성·수정·삭제해서는 안 된다.
- 공통 기능이 필요하면 `common-module`에 이미 있는 것을 사용하거나, 없으면 사용자에게 추가를 요청한다. 직접 추가하지 않는다.
